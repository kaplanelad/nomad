...
func (tr *TaskRunner) persistLocalState() error {
	...
	for name, val := range tr.localState.Hooks {
		tr.logger.Error("schmichael persisting ------->", "hook", name, "done", val.PrestartDone, "ndata", len(val.Data))
	}
...
func (tr *TaskRunner) Restore() error {
	...
	tr.logger.Error("schmichael localstate ============>", "numHooks", len(ls.Hooks))
	tr.localState = ls
	tr.state = ts
	return nil
}
